   ...
   stage ('Build') {
    ...
    stage ('Deploy-To-Tomcat') {
	 ...
	 stage ('Dynamic analysis') {
            steps {
           sshagent(['zap']) {
                sh 'ssh -o  StrictHostKeyChecking=no ubuntu@54.227.78.56 "sudo docker run --rm -v /home/ubuntu:/zap/wrk/:rw -t owasp/zap2docker-stable zap-full-scan.py -t http://3.85.32.238:8080/webapp -x zap_report  || true" '
